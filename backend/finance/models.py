...
def finance_invoice_pdf_upload_path(instance, filename):
    try:
        if instance.order and hasattr(instance.order, 'restaurant') and instance.order.restaurant:
            return f"restaurants/{instance.order.restaurant.slug}/finance_invoices/{filename}"
        elif instance.payroll and hasattr(instance.payroll, 'staff') and hasattr(instance.payroll.staff, 'restaurant') and instance.payroll.staff.restaurant:
            return f"restaurants/{instance.payroll.staff.restaurant.slug}/finance_invoices/{filename}"
    except Exception:
        pass
    return f"finance_invoices/misc/{filename}"
...
class Invoice(models.Model):
    ...
    pdf_file = models.FileField(upload_to=finance_invoice_pdf_upload_path, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
