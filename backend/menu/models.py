...
def product_image_upload_path(instance, filename):
    return f"restaurants/{instance.restaurant.slug}/products/{filename}"
...
class Category(models.Model):
    ...
    def active_products(self):
        """
        Returns active products using Django's prefetched cache if available
        to guarantee 0 database hits during list rendering.
        """
        if hasattr(self, '_prefetched_objects_cache') and 'products' in self._prefetched_objects_cache:
            return [p for p in self._prefetched_objects_cache['products'] if p.is_active]
...
class Product(models.Model):
    ...
    image = models.ImageField(_("Image"), upload_to=product_image_upload_path, null=True, blank=True)
    ...
    def average_rating(self):
        """
        Returns average rating. Checks for database annotated value first
        to bypass the review query completely during menu rendering.
        """
        if hasattr(self, 'annotated_rating'):
            return self.annotated_rating or 0.0
    ...
    def review_count(self):
        """
        Returns review count. Checks for database annotated value first
        to bypass the count query completely during menu rendering.
        """
        if hasattr(self, 'annotated_review_count'):
            return self.annotated_review_count
        from reviews.models import ProductReview
