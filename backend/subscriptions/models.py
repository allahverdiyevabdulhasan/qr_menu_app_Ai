...
class Subscription(models.Model):
    ...
    custom_discount_percent = models.DecimalField(_("Custom Discount (%)"), max_digits=5, decimal_places=2, default=0)
...
class BillingHistory(models.Model):
    restaurant = models.ForeignKey(Restaurant, on_delete=models.CASCADE, related_name='billing_history')
    ...
    def __str__(self):
        return f"{self.restaurant.name} - {self.amount_paid} {self.currency} on {self.payment_date.strftime('%Y-%m-%d')}"
